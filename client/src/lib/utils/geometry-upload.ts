import { load, parse, selectLoader } from "@loaders.gl/core";
...
export const validateGeoJSONSize = (
...
export const validateGeoJSONBounds = (
  ...
  const boundsPolygon = bboxPolygon(
    bounds.flat() as [number, number, number, number],
  );
  return booleanContains(boundsPolygon, geojson);
};
  ...
  switch (loader) {
    case KMLLoader: {
      ...
      try {
        ...
        const xmlWithError = new DOMParser().parseFromString(
          "invalid",
          "text/xml",
        );

        const parseErrorNS =
          xmlWithError.getElementsByTagName("parsererror")[0].namespaceURI;

        if (
          xml.getElementsByTagNameNS(parseErrorNS, "parsererror").length > 0
        ) {
...
export async function convertFilesToGeojson(
  ...
  let fileToParse: File | ArrayBuffer =
    files.find((f) => f.name.endsWith(".shp")) ?? files[0];
  ...
  if (fileToParse.name.endsWith(".kmz")) {
    ...
    const kmlFileName = Object.keys(fileMap).find((name) =>
      name.endsWith(".kml"),
    );
  ...
  try {
    content = (await load(fileToParse, loader, {
      ...
      ReturnType<typeof KMLLoader.parse | typeof ShapefileLoader.parse>
    >;
  } catch (_e) {
    return Promise.reject(UploadErrorType.UnsupportedFile);
  }
  ...
  if (loader === ShapefileLoader) {
    content = (content as Awaited<ReturnType<typeof ShapefileLoader.parse>>)
      .data[0];
  }
  ...
  if (
    ...
    !validateGeoJSONBounds(cleanedGeoJSON, options.bounds)
  ) {
...
function cleanupGeoJSON(geoJSON: GeoJSON): Feature<ValidGeometryType> | null {
  const isFeature = (geoJSON: GeoJSON): geoJSON is Feature =>
    geoJSON.type === "Feature";

  const isFeatureCollection = (
    geoJSON: GeoJSON,
  ): geoJSON is FeatureCollection => geoJSON.type === "FeatureCollection";
