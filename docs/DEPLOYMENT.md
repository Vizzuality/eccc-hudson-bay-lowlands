...
Deployment does **not** seed the database. No workflow step, lifespan handler, or container
entrypoint runs the seed, so a change to `data-processing/src/datasets/metadata.json` -- new
layers, renamed titles, edited colour ramps or legends -- deploys without any visible effect
until someone seeds by hand:
...
`SEED_SECRET` is generated by Terraform (`infrastructure/modules/env/api_env_vars.tf`) and
stored as a GitHub secret of the same name.

The seed upserts on the IDs supplied in the file, so it is safe to re-run: layer rows keep
their IDs and `config` is replaced wholesale. Adding `?delete_first=true` wipes the
`categories`, `datasets`, and `layers` tables before the upsert -- use it only to drop rows
that no longer appear in the file, since anything absent from the payload is destroyed.

See [Getting Started](GETTING_STARTED.md) for the local equivalents.
